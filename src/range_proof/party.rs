...
impl<'a> PartyAwaitingPosition<'a> {
    ...
    pub fn assign_position_with_rng<T: RngCore + CryptoRng>(
        ...
        let a_blinding = Fr::random(&mut *rng);
        ...
        for (G_i, H_i) in bp_share.G(self.n).zip(bp_share.H(self.n)) {
            ...
            let point = -H_i;
            let point = G1Projective::conditional_select(&point, G_i, v_i);
            A += point;
            i += 1;
        }

        let s_blinding = Fr::random(&mut *rng);
        let s_L: Vec<Fr> = (0..self.n).map(|_| Fr::random(&mut *rng)).collect();
        let s_R: Vec<Fr> = (0..self.n).map(|_| Fr::random(&mut *rng)).collect();

        // Compute S = <s_L, G> + <s_R, H> + s_blinding * B_blinding
        
        let points: Vec<_> = iter::once(self.pc_gens.B_blinding)
            .chain(bp_share.G(self.n).cloned())
            .chain(bp_share.H(self.n).cloned()).collect();
        let scalars: Vec<_> = iter::once(s_blinding).chain(s_L.iter().cloned()).chain(s_R.iter().cloned()).collect();
        ...
        if points.len() == 0 || scalars.len() == 0 {
            ...
            S = G1Projective::multi_exp(points.as_slice(),scalars.as_slice());
        }
...
impl<'a> PartyAwaitingBitChallenge<'a> {
    ...
    pub fn apply_challenge_with_rng<T: RngCore + CryptoRng>(
        ...
        let t_1_blinding = Fr::random(&mut *rng);
        let t_2_blinding = Fr::random(&mut *rng);
